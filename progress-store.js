(function () {
  ...
  window.lasProgress = { learntTopics: read, isLearnt: (topic) => read().includes(topic), markLearnt: (topic) => { const topics = read(); if (!topics.includes(topic)) { topics.push(topic); write(topics); } return topics; }, questionStats: readStats, recordQuestions, recordQuestionResults, needsFocus: (questionId) => readStats().incorrectQuestionIds.includes(questionId), revisionTime };
}());
