...
function renderDetailedLearnPage(learnData) {
  ...
  const sections = learnData.sections.map(([number, title, copy]) => `<section class="learn-section"><span class="learn-section-number">${number}</span><div><h2>${title}</h2><p>${copy}</p></div></section>`).join('');
