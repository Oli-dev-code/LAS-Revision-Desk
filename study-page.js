...
function renderDiagramTest(question) {
  ...
  const targets = question.diagram.labels.map((label) => `<div class="diagram-target" data-target-id="${label.id}" style="left:${label.x}%;top:${label.y}%;${label.targetWidth ? `width:${label.targetWidth}px;` : ''}"><span>Drop label</span></div>`).join('');
